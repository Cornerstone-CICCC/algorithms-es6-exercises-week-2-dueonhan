...
const checkAir = function (samples, threshold) {
  // Code here!
  let average = 0;
  let smapleLength = samples.length;

  const dirtYCount = samples.reduce((acc, sample) => {
    if (sample === "dirty") {
      acc++;
    }
    return acc;
  ...
  average = dirtYCount / smapleLength;
  average = average.toFixed(1) //decimal point changed 
